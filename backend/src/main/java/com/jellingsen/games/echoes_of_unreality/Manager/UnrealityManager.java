...
public class UnrealityManager {
    ...
    public Location generateRandomLocation() {
        ...
        return generatePartiallyRandomLocation(randomLocation, null); // randomize all fields
    }

    // PARTIALLY RANDOM LOCATION //

    public Location generatePartiallyRandomLocation(Location location, String[] locked) {
        System.out.println("  -->  generating partially random location " + location.name + " with locked fields: " + Arrays.toString(locked));
        Vector<String > lockedVector = new Vector<String>(Arrays.asList(locked));

        CompressedLocation uniqueLoc = generateUniqueCompressedNameAndType(location.name, lockedVector.contains("name"), location.type, lockedVector.contains("type"));
        if (uniqueLoc == null) { return null; }
        location.name = uniqueLoc.name;
        location.type = uniqueLoc.type;

        if (!lockedVector.contains("modifier")) { location.modifier = generateRandomLocationModifier(); } 
        if (!lockedVector.contains("size")) { location.size = generateRandomLocationSize(location.modifier); } // based on modifier (contrary or extreme)
        if (!lockedVector.contains("anomalies")) { location.anomalies = generateRandomLocationAnomalies(location.type, location.modifier); } // based on type and modifier (unreal) (use llm?)

        if (!lockedVector.contains("parent")) { 
        ...
        if (!lockedVector.contains("positionOnParent")) { location.positionOnParent = generateRandomPositionOnParent(location.type); } // based on type
        if (!lockedVector.contains("children")) { 
            ...
            location.children = joinAndRemoveDuplicates((existingChildren != null && !existingChildren.isEmpty()) ? existingChildren : generateRandomCompressedChildren(location.type), location.children); // based on type, adds any children passed in
        }

        boolean natureLocked = lockedVector.contains("nature");
        boolean societyLocked = lockedVector.contains("society");
        Location parentLocation = null; 
        if ((!natureLocked && (location.type.equals(LocationType.CONTINENT) || location.type.equals(LocationType.COUNTRY) || location.type.equals(LocationType.AREA) || location.type.equals(LocationType.CITY) || location.type.equals(LocationType.PLACE))) 
            || (!societyLocked && (location.type.equals(LocationType.CITY) || location.type.equals(LocationType.PLACE)))) 
        { parentLocation = uncompressLocation(location.parent); } // recursively generate or find parent until a society or nature to inherit is found
        if (!natureLocked) { location.nature = generateRandomLocationNature(location.type, location.modifier, location.size, location.parent.type, (parentLocation == null) ? null : parentLocation.nature, location.children); } // based on type, modifer (deserted or volatile or extreme), size, , children, & parentType (& if contains continent if star) (use llm?)
        if (!societyLocked) { location.society = generateRandomLocationSociety(uniqueLoc, location.modifier, location.anomalies, location.nature, (parentLocation == null) ? null : parentLocation.society, (parentLocation == null) ? null : parentLocation.children); } // based on type and modifier (deserted or extreme), anomalies, environments, & siblings (use llm?) 
        
        if (!lockedVector.contains("appearance")) { location.appearance = generateRandomAppearance(location.type, location.size, location.nature, location.parent.type); } // based on type, size, and materials (use llm?)
        if (!lockedVector.contains("summary")) { location.summary = generateRandomLocationSummary(location.type, location.anomalies, location.society); } // based on type, anomalies, culture, & secrets (use llm?)
    ...
    private Vector<CompressedLocation> joinAndRemoveDuplicates(Vector<CompressedLocation> children, Vector<CompressedLocation> andChildren) {
    ...
    private LocationNature generateRandomLocationNature(LocationType type, LocationModifier modifier, LocationSize size, LocationType parentType, LocationNature parentNature, Vector<CompressedLocation> children) {
        ...
        switch (type) {
            ...
            case AREA -> {
                if (LocationType.FEATURE.equals(parentType)) {
                    ln = new LocationNature();
                    ln.setupNature(type, modifier, size, true); 
                }
            }
            default -> { ln = inheritNatureFromParent(parentNature, modifier); } 
        }
        return ln;
    }
    ...
    private LocationSociety generateRandomLocationSociety(CompressedLocation currentLoc, LocationModifier modifier, Vector<String> anomalies, LocationNature nature, LocationSociety parentSociety, Vector<CompressedLocation> tooManySiblings) { 
                if (modifier != null && modifier.equals(LocationModifier.DESERTED)) { return null;}
                ...
                switch (currentLoc.type) {
                    ...
                    case CITY,PLACE -> chanceOfSociety = -1; // inheritFromParent 
                }
    ...
    private LocationSociety inheritSocietyFromParent(LocationSociety parentSociety, LocationModifier modifier) {
        if (LocationModifier.DESERTED.equals(modifier)) {
            return null;
        } else {
            if (LocationModifier.EXTREME.equals(modifier)) {
                // josh potential llm
    ...
    private Location uncompressLocation(CompressedLocation compressedLocation) {
        ...
        l = generatePartiallyRandomLocation(l, new String[] {"name", "type"}); // will find & link exisiting parent or children automatically
...
//     // location functions continued
    ...
    //     Location currentLocation = databaseConnection.getLocation(key);

    //     Vector<Location> tempChildren = new Vector<Location>();
    //     for (CompressedLocation cLoc : databaseConnection.getLocationsChildren(key)) {
    //         tempChildren.add(uncompressChild(cLoc, compressLocation(currentLocation), currentLocation.modifier, currentLocation.society));
    //     }
